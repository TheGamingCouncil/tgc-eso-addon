...
GetFastTravelNodeInfo = function(nodeIndex, ...)
  ...
  if GetMapType() == MAPTYPE_WORLD then
    if poiType == POI_TYPE_HOUSE and TGC.db.default.options.mapFlags.homePreviews ~= true and icon == "/esoui/art/icons/poi/poi_group_house_unowned.dds" then
      return known, name, normalizedX, normalizedY, icon, glowIcon, poiType, false, linkedCollectibleIsLocked
    end
    if poiType == POI_TYPE_HOUSE and TGC.db.default.options.mapFlags.ownedHomes ~= true and icon == "/esoui/art/icons/poi/poi_group_house_owned.dds" then
      return known, name, normalizedX, normalizedY, icon, glowIcon, poiType, false, linkedCollectibleIsLocked
    end

    if poiType == 1 and TGC.db.default.options.mapFlags.waypoints ~= true then
      return known, name, normalizedX, normalizedY, icon, glowIcon, poiType, false, linkedCollectibleIsLocked
    end
    if poiType == 3 and TGC.db.default.options.mapFlags.trials ~= true then
      return known, name, normalizedX, normalizedY, icon, glowIcon, poiType, false, linkedCollectibleIsLocked
    end
    if poiType == 6 and TGC.db.default.options.mapFlags.dungeons ~= true then
      return known, name, normalizedX, normalizedY, icon, glowIcon, poiType, false, linkedCollectibleIsLocked
    end
